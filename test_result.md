...
agent_communication:
    ...
    -agent: "main"
    -message: "ITER6: Fixed root cause of 'In preparazione' never publishing — senseImageBase64 now uses fetch(file://)->base64 fallback so publish is immediate when image exists (queue only for real network errors). Pulse now expire 24h end-to-end (pulse_expires_at + pulse_feed filters created_at>=now-24h AND has_image), imageless pulses rejected + cleaned on startup. Pulse viewer three-dots menu + inline delete confirmation implemented. Testing agent iter6: backend 6/6 PASS, frontend menu/confirm/one-ring PASS. Publish end-to-end capture is device-only → validate on TestFlight build."

    -agent: "main"
    -message: "PUBLISH TRACE (iter7): Confirmed end-to-end server-side works — POST /observations (is_pulse) returns 200 with id+image_url, media stored (GET /media 200), appears in /feed AND /pulse/feed with image. The ONLY failure point is CLIENT-SIDE image read (senseImageBase64 returned null on device → createObservation never called → masked by 'In preparazione' silent queue). Fixes: (1) imageUpload.loadSenseImage now tries 2 resize sizes (1440,900) then raw file read, returning a per-strategy DIAGNOSTIC on failure; (2) composer shows REAL states (Caricamento/Pubblicato/Errore-con-motivo/Riprova), no silent queue as default — network errors are queued visibly with Retry; (3) image-read failure now shows explicit technical reason instead of 'In preparazione'. Device-only, requires rebuild to validate."

    -agent: "main"
    -message: "ITER8: Unique nicknames (real-time availability NicknameField + backend endpoint), immutable unique 3-char author_code (register retry + idempotent backfill, EXP/NEO), new SenseShot ID #NEO-000000021 (data.senseCode), and Search/Explore section (GET /api/search keyword+NLP, 5-col grid, infinite scroll, pull-to-refresh, SenseQuickView -> full Observe card). Bottom nav now 7 slots (Home/Search/Pulse/SenseVision center/Observe/Messages/Gallery). Testing agent iter7: backend 14/14 PASS, frontend register/search/nav PASS. Publish + personal Gallery are device-only (not web-tested)."

    -agent: "main"
    -message: "ITER9 (Observe vs Observe World): FIXED empty Observe feed. Root cause: smart feed's seen_factor dropped observations the viewer already saw; after publishing you open the post (marked seen) so YOUR own just-published Sense was excluded from YOUR Observe feed, while Observe World (curated, no seen-filter) still showed it — hence the confusion. Fix: (1) smart_score now ALWAYS keeps the viewer's OWN observations (never hidden by seen/impression), (2) smart feed falls back to most-recent when the seen logic would otherwise empty the feed. Verified: explorer sees own 20 posts; 5 remain visible even after markSeen. Observe World correctly curates via reality_score>=MIN_WORLD_SCORE (architecture confirmed). Publish-original-image failure is device-only; loadSenseImage keeps per-strategy diagnostics for the next build."
