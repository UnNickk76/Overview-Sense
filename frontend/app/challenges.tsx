...
export default function Challenges() {
  ...
  const [global, setGlobal] = useState<{ pulse: GlobalPulse; participants: number } | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [f, g] = await Promise.all([
        pulseApi.feed(),
        pulseApi.globalActive().catch(() => null),
      ]);
      setItems(f.items);
      setGlobal(g);
    } catch { setItems([]); }
  ...
  const shuffle = () => { Haptics.selectionAsync(); setSalt((s) => s + 1); };

  const togglePick = (id: string) => {
    Haptics.selectionAsync();
    setPicks((p) => (p.includes(id) ? p.filter((x) => x !== id) : p.length >= 2 ? [p[1], id] : [...p, id]));
  };
  ...
  const SoonRow = ({ icon, title, sub }: { icon: keyof typeof Ionicons.glyphMap; title: React.ReactNode; sub: string }) => (
    <View style={styles.soonRow}>
      <View style={styles.soonIcon}><Ionicons name={icon} size={20} color={colors.onSurfaceSecondary} /></View>
      <View style={{ flex: 1 }}>
        <Text style={styles.soonTitle}>{title}</Text>
        <Text style={styles.soonSub}>{sub}</Text>
      </View>
      <Text style={styles.soonTag}>Presto</Text>
    </View>
  );
  ...
  return (
    <SpaceBackground>
      ...
      <ScrollView
        contentContainerStyle={{ paddingHorizontal: spacing.lg, paddingTop: spacing.md, paddingBottom: insets.bottom + 130, gap: spacing.lg }}
        showsVerticalScrollIndicator={false}
        refreshControl={<RefreshControl refreshing={loading} onRefresh={load} tintColor={colors.brand} />}
      >
        {/* Global Pulse */}
        {global?.pulse ? (
          <Pressable testID="global-pulse-card" style={styles.globalCard}
            onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); router.push(`/pulse-global?id=${global.pulse.id}` as never); }}>
            <View style={styles.globalHead}>
              <Text style={styles.globalIcon}>🌍</Text>
              <View style={{ flex: 1 }}>
                <Text style={styles.globalOverline}>PULSE GLOBALE · TUTTO IL MONDO</Text>
                <Text style={styles.globalTitle}>{global.pulse.title}</Text>
              </View>
              <Ionicons name="chevron-forward" size={20} color={colors.brand} />
            </View>
            <Text style={styles.globalPrompt} numberOfLines={2}>{global.pulse.prompt}</Text>
            <View style={styles.globalMeta}>
              <View style={styles.globalPill}>
                <Ionicons name="people" size={12} color={colors.brand} />
                <Text style={styles.globalPillText}>{global.participants} {global.participants === 1 ? "osservatore" : "osservatori"}</Text>
              </View>
              <Text style={styles.globalTime}>Termina tra {endsIn(global.pulse.ends_at)}</Text>
            </View>
          </Pressable>
        ) : null}
            ...
            {items.map((o) => {
              ...
              return (
                <Pressable key={o.id} testID={`pulse-item-${o.id}`} style={[styles.gridItem, { width: col }]}
                  ...
                  {compareMode ? (
                    <View style={[styles.pick, picked && styles.pickOn]}>{picked ? <Ionicons name="checkmark" size={14} color={colors.onBrand} /> : null}</View>
                  ) : null}
        ...
        {/* Coming soon — the wider Challenges universe */}
        <Text style={styles.sectionTitle}>Altre sfide</Text>
        <SoonRow icon="people-outline" title={<BrandName name="Observe" style={styles.soonTitle} />} sub="Sfide tra Observer di tutto il mondo" />
        <SoonRow icon="person-add-outline" title="Sfide tra amici" sub="Sfida chi segui a osservare lo stesso tema" />
        <SoonRow icon="trophy-outline" title="Classifiche" sub="I migliori osservatori della settimana" />
        <SoonRow icon="calendar-outline" title="Eventi speciali" sub="Eclissi, sciami meteorici, aurore" />
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  globalCard: { backgroundColor: "rgba(212,175,55,0.08)", borderRadius: radius.lg, padding: spacing.lg, borderWidth: 1, borderColor: colors.brand, gap: spacing.sm },
  globalHead: { flexDirection: "row", alignItems: "center", gap: spacing.md },
  globalIcon: { fontSize: 30 },
  globalOverline: { color: colors.brand, fontFamily: fonts.bold, fontSize: type.sm - 3, letterSpacing: 1 },
  globalTitle: { color: colors.onSurface, fontFamily: fonts.bold, fontSize: type.lg, marginTop: 1 },
  globalPrompt: { color: colors.onSurface, fontFamily: fonts.regular, fontSize: type.sm, lineHeight: 20 },
  globalMeta: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 2 },
  globalPill: { flexDirection: "row", alignItems: "center", gap: 5, backgroundColor: colors.surfaceTertiary, borderRadius: 999, paddingHorizontal: spacing.sm, paddingVertical: 4 },
  globalPillText: { color: colors.brand, fontFamily: fonts.semibold, fontSize: type.sm - 2 },
  globalTime: { color: colors.onSurfaceSecondary, fontFamily: fonts.mono, fontSize: type.sm - 2 },
  ...
  soonRow: { flexDirection: "row", alignItems: "center", gap: spacing.md, backgroundColor: colors.surfaceSecondary, borderRadius: radius.md, padding: spacing.md, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.border },
  soonIcon: { width: 40, height: 40, borderRadius: 20, alignItems: "center", justifyContent: "center", backgroundColor: colors.surfaceTertiary },
  soonTitle: { color: colors.onSurface, fontFamily: fonts.semibold, fontSize: type.base },
  soonSub: { color: colors.onSurfaceSecondary, fontFamily: fonts.regular, fontSize: type.sm - 1, marginTop: 1 },
  soonTag: { color: colors.onSurfaceTertiary, fontFamily: fonts.medium, fontSize: type.sm - 2, backgroundColor: colors.surfaceTertiary, borderRadius: 999, paddingHorizontal: spacing.sm, paddingVertical: 3 },
